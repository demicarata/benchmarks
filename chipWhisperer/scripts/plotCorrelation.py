...
def main():
    ...
    # Test for leakage of x0 and x1 themselves with each operation
    hw_x0 = hw(x0)
    hw_x1 = hw(x1)

    corr_secret = correlate_with_traces(hw_secret, traces)  
    corr_x0     = correlate_with_traces(hw_x0,     traces)  
    corr_x1     = correlate_with_traces(hw_x1,     traces)  

    subtitle_parts = [effect, chip, f"run {index}", f"{num_traces} traces"]
    if variant:
        subtitle_parts.insert(1, variant)
    ...
    axes[0].set_title("HW(x0) — expected leakage at first instruction")

    axes[1].plot(samples, corr_x1, color='darkorange')
    axes[1].axhline(0, color='k', linewidth=0.5)
    axes[1].set_ylabel("Correlation")
    axes[1].set_title("HW(x1) — expected leakage at second instruction")

    axes[2].plot(samples, corr_secret, color='crimson')
    axes[2].axhline(0, color='k', linewidth=0.5)
    axes[2].set_ylabel("Correlation")
    axes[2].set_title("HW(secret) = HW(x0 XOR x1)")
    ...
    if variant:
        plot_dir = os.path.join("plots", effect, variant, chip)
    else:
        plot_dir = os.path.join("plots", effect, chip)
